...
app_function (void *userdata)
{
  ...
  /* Build pipeline */
  data->pipeline =
          gst_parse_launch ("udpsrc port=8080 caps = \"application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, payload=(int)96\" !  rtph264depay ! decodebin ! videoconvert ! autovideosink",
                            &error);
  if (error) {
    gchar *message =
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
  ...
  jclass klass = (*env)->FindClass (env,
                                    "org/freedesktop/gstreamer/tutorials/tutorial_3/Fragment2");
